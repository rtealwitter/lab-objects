...
The `ls` command does not show hidden files, but the `ls -a` command does.
...
> **NOTE:**
> Filenames are customarily enclosed in backticks,
> but ALLCAPS filenames are often not put in backticks
> (because ALLCAPS is a historic shorthand from the 80s for an inline code block).
> Thus, it is correct to write either README or `README`.
> The non-allcaps version README can refer to a file named `README` (with no extension, not markdown formatted)
>  or a file named `README.md` (with the `.md` markdown extension).
...
Create a file `message.py` with the following code:
